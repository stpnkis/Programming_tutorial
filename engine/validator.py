...
LESSON_SCHEMA = {
    "required": ["id", "title"],
    "optional": [
        "summary", "difficulty", "estimated_minutes", "tags",
        "prerequisites", "learning_objectives", "challenges",
        "why_it_matters", "what_you_will_learn", "key_theory",
        "lesson_summary", "recommended_next", "before_you_code",
    ],
    "types": {
        "id": str,
        "title": str,
...
CHALLENGE_SCHEMA = {
    ...
    "optional": [
        ...
        "tags", "estimated_minutes", "learning_objective",
        "prerequisites", "hint_strategy", "review_priority",
        "expected_misconceptions", "mastery_rule", "solution_pattern",
        # Rich pedagogical fields
        "why_it_matters", "what_you_will_learn", "key_concept",
        "worked_example", "common_mistakes", "thinking_notes",
        "reference_solution", "solution_explanation", "practice_mode",
    ],
    "valid_types": ["implementation", "knowledge", "debugging", "refactoring", "trace", "open"],
...
class LessonQuality:
    ...
    def metadata_score(self) -> float:
        ...
        if self.challenge_count > 0:
            ...
            pts += 4 * min(self.challenges_with_misconceptions / self.challenge_count, 1.0)
        return round(pts, 1)
    ...
    def pedagogy_score(self) -> float:
        ...
        if self.challenge_count > 0:
            ...
            pts += 4 * min(self.challenges_with_quality_hints / self.challenge_count, 1.0)
            # Non-skeleton challenge bodies
            pts += 4 * min(self.func_body_quality / self.challenge_count, 1.0)
            # Worked examples
            pts += 4 * min(self.challenges_with_worked_example / self.challenge_count, 1.0)
            # Thinking notes
            pts += 2 * min(self.challenges_with_thinking_notes / self.challenge_count, 1.0)
            # Reference solutions
            pts += 3 * min(self.challenges_with_reference_solution / self.challenge_count, 1.0)
            # Solution explanations (new — ensures reference has reasoning)
            pts += 3 * min(self.challenges_with_solution_explanation / self.challenge_count, 1.0)
            # Common mistakes coverage
            pts += 2 * min(self.challenges_with_common_mistakes / self.challenge_count, 1.0)
    ...
    def score(self) -> float:
        ...
        return round(self.structure_score + self.metadata_score + self.pedagogy_score, 1)
...
def validate_challenges_file(filepath: str, result: ValidationResult,
    ...
    if func_count < challenge_count:
        result.warn(rel, f"{challenge_count} Challenge() but only {func_count} vyzva_ functions")
...
def validate_lesson_yaml(yaml_path: str, challenges_path: str,
    ...
    for f_name, expected_type in LESSON_SCHEMA["types"].items():
        if f_name in data and not isinstance(data[f_name], expected_type):
            result.error(rel, f"Field '{f_name}' must be {expected_type.__name__}, "
    ...
    if quality:
        ...
        quality.has_quality_summary = (
            bool(summary_text) and len(summary_text) > 15
        ...
        # Lesson-level pedagogical content
        quality.has_why_it_matters = bool(data.get("why_it_matters")) and len(str(data.get("why_it_matters", ""))) > 20
        quality.has_what_you_will_learn = bool(data.get("what_you_will_learn")) and len(str(data.get("what_you_will_learn", ""))) > 20
        quality.has_key_theory = bool(data.get("key_theory")) and len(str(data.get("key_theory", ""))) > 20
        quality.has_lesson_summary = bool(data.get("lesson_summary")) and len(str(data.get("lesson_summary", ""))) > 20
        quality.has_recommended_next = bool(data.get("recommended_next")) and len(str(data.get("recommended_next", ""))) > 10
        quality.has_before_you_code = bool(data.get("before_you_code")) and len(str(data.get("before_you_code", ""))) > 20
    ...
    if "challenges" in data and isinstance(data["challenges"], list):
        ...
        for i, ch in enumerate(data["challenges"]):
            ...
            if "hint_strategy" in ch:
                if ch["hint_strategy"] not in CHALLENGE_SCHEMA["valid_hint_strategies"]:
                    result.warn(
                        rel, f"Challenge {i+1} unknown hint_strategy: "
                             f"{ch['hint_strategy']}")
            if "review_priority" in ch:
                if ch["review_priority"] not in CHALLENGE_SCHEMA["valid_review_priorities"]:
                    result.warn(
                        rel, f"Challenge {i+1} unknown review_priority: "
                             f"{ch['review_priority']}")
            if "mastery_rule" in ch:
                if ch["mastery_rule"] not in CHALLENGE_SCHEMA["valid_mastery_rules"]:
                    result.warn(
                        rel, f"Challenge {i+1} unknown mastery_rule: "
                             f"{ch['mastery_rule']}")
            ...
            if quality:
                ...
                if ch.get("reference_solution") and len(str(ch["reference_solution"])) > 10:
                    quality.challenges_with_reference_solution += 1
                if ch.get("solution_explanation") and len(str(ch["solution_explanation"])) > 15:
            ...
            if "practice_mode" in ch:
                if ch["practice_mode"] not in CHALLENGE_SCHEMA["valid_practice_modes"]:
                    result.warn(
                        rel, f"Challenge {i+1} unknown practice_mode: "
                             f"{ch['practice_mode']}")
        ...
        if os.path.isfile(challenges_path):
            ...
            if actual != declared:
                result.warn(rel, f"YAML declares {declared} challenges but "
...
def validate_section(section_path: str, result: ValidationResult):
    ...
    for lesson_dir in lesson_dirs:
        ...
        if os.path.isfile(challenges_file):
            ...
            result.warn(
                os.path.relpath(lesson_path, ROOT),
                "No challenges.py found"
            )
...
def format_quality_report(result: ValidationResult) -> str:
    ...
    lines.append("  ╔══════════════════════════════════════════════════════"
                 "══════════════════════════╗")
    lines.append("  ║  QUALITY REPORT — Structure | Metadata | Pedagogy   "
                 "                          ║")
    lines.append("  ╠══════╦═══════════════════════════════════╦═══════╦═══"
                 "════╦════════╦══════════╣")
    lines.append("  ║ Sec  ║ Lesson                            ║ Str   ║ "
                 "Meta  ║ Ped    ║ Total    ║")
    lines.append("  ╠══════╬═══════════════════════════════════╬═══════╬═══"
                 "════╬════════╬══════════╣")
    ...
    for sec_num, avg in section_scores:
        ...
        g = "A" if avg >= 80 else "B" if avg >= 60 else "C" if avg >= 40 else "D" if avg >= 20 else "F"
    ...
    if all_scores:
        ...
        lines.append(
            f"  Grade A (≥80): {sum(1 for s in all_scores if s >= 80)}")
        lines.append(
            f"  Grade F (<20): {sum(1 for s in all_scores if s < 20)}")
...
def format_qa_report(result: ValidationResult) -> str:
    ...
    for q in sorted_q[:15]:
        lines.append(f"  ── {q.section_num}.{q.lesson_num} {q.path} "
    ...
    if total > 0:
        ...
        if skeleton_only > 0:
            lines.append(f"     1. Write real challenge bodies for {skeleton_only} skeleton lessons")
        if no_objectives > 0:
...
def _get_improvement_suggestions(q: LessonQuality) -> List[str]:
    ...
    if not q.has_quality_summary:
        suggestions.append("Add meaningful summary (>15 chars, describe what student learns)")
    ...
    if q.challenge_count > 0 and q.challenges_with_solution_explanation == 0:
        suggestions.append("Add solution_explanation to challenges — explain WHY, not just WHAT")
    ...
    if q.type_diversity <= 1 and q.challenge_count >= 3:
        suggestions.append("Use diverse challenge types (debugging, knowledge, trace, open)")
    if q.challenge_count >= 4 and q.trivial_challenge_count > q.challenge_count * 0.6:
        suggestions.append("⚠️  Too many trivial challenges — add harder/practical tasks")
    if q.challenge_count >= 3 and not q.has_reasoning_task:
        suggestions.append("Add at least one reasoning task (trace, knowledge, or debugging)")
    return suggestions
